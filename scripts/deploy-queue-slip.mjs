...
const source = await readFile(resolve(process.cwd(), "contracts/QueueSlip.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "QueueSlip.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["QueueSlip.sol"]?.QueueSlip;
if (!contract) {
...
console.log(`Deploying QueueSlip from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_QUEUE_SLIP_CONTRACT_ADDRESS to this contract address in Vercel.");
